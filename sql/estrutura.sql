...
CREATE TABLE IF NOT EXISTS `sgc`.`LISTA_PRODUTOS` (
  ...
  `OS_ID` INT NULL,
  `LIST_PED_QNT` DECIMAL(10,2) NULL,
...
-- -----------------------------------------------------
-- Table `sgc`.`DESPESA/RECEITA`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `sgc`.`DESPESA/RECEITA` (
  `DESREC_ID` INT NOT NULL AUTO_INCREMENT,
  `USUARIO_ID` INT NULL,
  `PEDIDO_ID` INT NULL,
  `DESCRE_NATUREZA` ENUM('1','2') NOT NULL COMMENT '1 - RECEITA\n2 - DESPESA',
  `DESREC_DESCR` VARCHAR(45) NULL,
  `DESREC_VALOR` DECIMAL(10,2) NOT NULL,
  `DESREC_VECIMENTO` DATE NOT NULL,
  `DESCRE_DATA` DATETIME NOT NULL,
  `DESCRE_ESTATUS` ENUM('ab','pg') NOT NULL COMMENT 'A = aberta\nP = Pago',
  PRIMARY KEY (`DESREC_ID`),
  INDEX `fk_DESP_RECEITA_USUARIOS1_idx` (`USUARIO_ID` ASC),
  INDEX `fk_DESPESA/RECEITA_PEDIDOS1_idx` (`PEDIDO_ID` ASC),
  CONSTRAINT `fk_DESP_RECEITA_USUARIOS1`
    FOREIGN KEY (`USUARIO_ID`)
    REFERENCES `sgc`.`USUARIOS` (`USUARIO_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_DESPESA/RECEITA_PEDIDOS1`
    FOREIGN KEY (`PEDIDO_ID`)
    REFERENCES `sgc`.`PEDIDOS` (`PEDIDO_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
